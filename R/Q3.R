...
#' q3obj <- Q3(items = it.SPADI,
#'             method.item = "CML", method.person = "WML",
#'             model = "RMP",
#'             standardize = FALSE)
#'
#' @export Q3
#'
Q3 <- function(items, method.item = c("PCML", "CML", "JML", "MML"), method.person = c("WML", "MLE"), model = c("RMD", "RMP"), standardize = TRUE, ...) {
  ...
  resids[is.infinite(resids)] <- NA
  Q3matrix <- cor(x = resids, method = "pearson", use = "pairwise.complete.obs")
