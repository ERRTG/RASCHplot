...
#'
#' @param p A \link[RASCHplot]{ggPImap} to add marginal plots to. If \code{p} is
#' not provided, then all of \code{theta}, \code{beta}, and \code{threshtable} must be
#' provided.
#' @param theta The person parameters to use for creating the marginal plots. Or a \code{eRm} or \code{Rm} object -- result of \link[eRm]{RM} or \link[eRm]{PCM}.
#' @param beta The item parameters. This must be a vector or list of vectors in case of stratification. Ignored if \code{p} is provided.
#' @param threshtable The item-category threshold parameters. This must be a matrix or data frame or list of matrices/data frames in case of stratification. Ignored if \code{p} is provided.
#' @param group group
...
#' p <- ggPImap(object)
#' ggPImap_hist(p, theta)
#'
#' ggPImap_hist(p, theta, group = as.factor(SPADI.complete$gender[-pp$pers.ex]))
#'
#' if (FALSE) {
...
#' p <- ggPImap(objlist, legend.title = "Gender", legend.labels = c("Male", "Female"))
#' ggPImap_hist(p, theta = list(theta1 = theta1, theta2 = theta2),
#'              legend.title = "Gender", legend.labels = c("1", "2"))
#' }
#' @export
#'
ggPImap_hist <- function(p, theta, beta, threshtable,
                         group = NULL,
                         type = c("histogram", "density", "boxplot", "violin",
                                  "densigram"),
                         margins = c("top", "right", "bottom", "left"), size = 5, ggtheme = theme_void, ..., xparams = list(), yparams = list(),
  ...
  scatP <- .reconcileScatPlot(p, beta, threshtable) +
  ...
  #if (hasTitle) {
  #  ggPImap_hist <- addTitleGrobs(ggxtraNoTtl, titleGrobs)
  #} else {
    ggPImap_hist <- ggxtraNoTtl
  #}

  # Add a class for S3 method dispatch for printing the ggExtra plot
  class(ggPImap_hist) <- c("ggPImap_hist", class(ggPImap_hist))

  ggPImap_hist
}
#' Print a ggPImap_hist object
#'
#' \code{ggPImap_hist} objects are created from \code{ggPImap_hist}. This is the S3
#' generic print method to print the result of the scatterplot with its marginal
#' plots.
#'
#' @param x ggPImap_hist object.
#' @param newpage Should a new page (i.e., an empty page) be drawn before the
#' ggPImap is drawn?
#' @param ... ignored
#' @seealso \code{\link{ggPImap}}
#' @export
#' @keywords internal
print.ggPImap_hist <- function(x, newpage = grDevices::dev.interactive(), ...) {
...
#' @param p p
#' @param beta beta
#' @param threshtable threshtable
.reconcileScatPlot <- function(p, beta, threshtable) {
  if (missing(p)) {
    if (missing(beta) || missing(threshtable)) {
      stop("`beta` and `threshtable` must be provided if `p` is not provided",
           call. = FALSE
      )
    }
    p <- ggPImap(beta = beta, threshtable = threshtable)
  }
  p
}
